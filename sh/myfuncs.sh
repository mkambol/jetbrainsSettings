...
sdbg() {
    ...
    export OPT="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5009"
        ...
        sed -i -e "s/featuresBoot=config/featuresBoot=ssh,config/g" system/karaf/etc/org.apache.karaf.features.cfg
...
# AEL stuff
#server=devuser@svqxbdcn6cdh512n1
#diPath=/home/devuser/matt/data-integration
port=2022
server=cloudera@mayhem.local
diPath=/home/cloudera/pentaho/data-integration
localDi=/Users/mcampbell/pentaho/latest/data-integration
adaptiveExec=/Users/mcampbell/dev/pentaho-ee/adaptive-execution
version=8.1.0.0-SNAPSHOT
sparkDir=/Users/mcampbell/pentaho/spark

function copyToServer() {
    
    scp  -P ${port} ~/dev/pentaho-ee/adaptive-execution/pdi-spark-app/pdi-spark-engine-operations/target/pdi-spark-engine-operations-${version}.jar  ${server}:${diPath}/system/karaf/system/org/pentaho/adaptive/pdi-spark-engine-operations/${version}
    scp  -P ${port} ~/dev/pentaho-ee/adaptive-execution/pdi-engines/pdi-engine-spark/target/pdi-engine-spark-${version}.jar  ${server}:${diPath}/lib
    ssh ${server}  -p ${port} rm -rfv ${diPath}/system/karaf/caches
}


function updateSparkExecutor() {
    ssh  ${server} -p ${port} "cd $diPath/..;
...
function uploadFreshAEL() {
    ...
    scp -P 2022 pdi-spark-driver.zip ${server}:${diPath}/..
    ssh  ${server} -p 2022 "cd pentaho;
         rm -rf data-integration;
         rm -rf pdi-spark-executor.zip;
...
function buildAEL() {
   cd ${adaptiveExec}/pdi-engines/pdi-engine-spark
    mvn clean install -Dmaven.test.skip=true 
    cd ../../pdi-spark-app/pdi-spark-engine-operations/
    mvn clean install -Dmaven.test.skip=true  

}

function updateAELLocal() {
buildAEL()
yes | cp -f ${adaptiveExec}/pdi-engines/pdi-engine-spark/target/pdi-engine-spark-${version}.jar ${localDi}/lib/pdi-engine-spark-${version}.jar
yes | cp -f ${adaptiveExec}/pdi-spark-app/pdi-spark-engine-operations/target/pdi-spark-engine-operations-${version}.jar  ${localDi}/system/karaf/system/org/pentaho/adaptive/pdi-spark-engine-operations/${version}/pdi-spark-engine-operations-${version}.jar
rm -rf ${localDi}/system/karaf/caches

cd ${localDi}/adaptive-execution
./daemon.sh

}

function aelLocalConfig() {
    sed -i -e "s/driverDebugPort=-1/driverDebugPort=5007/g"  ${localDi}/adaptive-execution/config/application.properties
    sed -i -e "s#sparkHome=[^$]*#sparkHome=${sparkDir}#g"  ${localDi}/adaptive-execution/config/application.properties
    sed -i -e "s#sparkApp=[^$]*#sparkApp=${localDi}#g"  ${localDi}/adaptive-execution/config/application.properties
    sed -i -e "s#hadoopConfDir=[^$]*#hadoopConfDir=#g"  ${localDi}/adaptive-execution/config/application.properties
}
